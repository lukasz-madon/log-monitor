...
class LogFileProcessor(PatternMatchingEventHandler):
    ...
    """
    def __init__(
        self, log_file, offset_file, paranoid, traffic_limit, stats,
        msg_queue, patterns=None
    ):
    ...
    def read_log(self):
        ...
        for line in Pygtail(self.log_file, self.offset_file, self.paranoid):
            ...
            while self.log_queue and (end_time - start_time).seconds > ALERT_WINDOW:
                self.log_queue.pop()
                start_time = self.log_queue[-1].timestamp
...
def parse_log_line(line):
    ...
    if not match:
        ValueError(f'incorrect log format: {line}')
